...
class CF:
    def __init__(self,mol,method,rho,grad,tau,eps_x_exact,br03_params):
        ...
        self.method=method
        self.rho_up = rho["up"]
        self.rho_down = rho["down"]
        self.rho_tot=rho["tot"]
        self.zeta = (self.rho_up-self.rho_down)/self.rho_tot
        self.rs = (3./(4.*np.pi*self.rho_tot))**(1./3.)
        self.kf=(3.*np.pi**2*self.rho_tot)**(1./3.)
        self.br03_a_up=br03_params["up"]["a"]
        self.br03_b_up=br03_params["up"]["b"]
        self.br03_c_up=br03_params["up"]["c"]
        self.br03_n_up=br03_params["up"]["n"]

        self.br03_a_down=br03_params["down"]["a"]
        self.br03_b_down=br03_params["down"]["b"]
        self.br03_c_down=br03_params["down"]["c"]
        self.br03_n_down=br03_params["down"]["n"]
        ...
        self.eps_x_LSD_up = dft.libxc.eval_xc("LDA,", [self.rho_up,zeros],spin=5)[0]
        self.eps_x_LSD_down = dft.libxc.eval_xc("LDA,", [zeros,self.rho_down],spin=5)[0]
        self.eps_c_LSD = dft.libxc.eval_xc(",PW_mod",[self.rho_up,self.rho_down],spin=5)[0]
        self.eps_xc_LSD = (self.eps_x_LSD_up*self.rho_up+self.eps_x_LSD_down*
                                self.rho_down)/self.rho_tot + self.eps_c_LSD
        # to obtain pbe energy densities
        gga_up = [self.rho_up,grad["up"][0],grad["up"][1],grad["up"][2]]
        gga_down = [self.rho_down,grad["down"][0],grad["down"][1],grad["down"][2]]
        self.eps_x_PBE_up= dft.libxc.eval_xc("PBE,", [gga_up,[zeros,zeros,zeros,zeros]],spin=5)[0]
        self.eps_x_PBE_down= dft.libxc.eval_xc("PBE,", [[zeros,zeros,zeros,zeros],gga_down],spin=5)[0]
        self.eps_c_PBE = dft.libxc.eval_xc(",PBE",[gga_up,gga_down],spin=5)[0]
